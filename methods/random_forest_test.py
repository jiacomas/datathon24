...
if __name__ == "__main__":
    ...
    file2 = '/Users/luke/datathon24/no_bodegon.csv'
    ...
    data_clean2 = get_numeric_data(file2, ['des_color', 'des_sex', 'des_age', 'des_line', 'des_fabric', 
        'des_product_category', 'des_product_aggregated_family', 'des_product_family', 
        'des_product_type', 'silhouette_type', 'neck_lapel_type', 'woven_structure',
        'knit_structure', 'heel_shape_type', 'length_type', 'sleeve_length_type',
        'toecap_type', 'waist_type', 'closure_placement', 'cane_height_type'])
    ...
    Y = data_clean[labels] 
    X2 = data_clean2.drop(labels + ["cod_modelo_color", "des_filename", "cod_color"], axis=1)
    Y2 = data_clean2[labels] 
    ...
    multioutput_model.fit(X_train2, Y_train2)
    Y_pred2 = multioutput_model.predict(X_test2)
    ...
    accuracies2 = [accuracy_score(Y_test2.iloc[:, i], Y_pred2[:, i]) for i in range(len(labels))]
    print("Precisión global promedio:", (sum(accuracies)+sum(accuracies2)) / (len(accuracies)+len(accuracies2)))
